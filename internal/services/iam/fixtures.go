...
import (
	...
	acctest2 "github.com/hashicorp/terraform-plugin-sdk/v2/helper/acctest"
...
func CreateFakeIAMManager(tt *acctest.TestTools) (*account.Project, *iamSDK.APIKey, FakeSideProjectTerminateFunc, error) {
	...
	projectName := acctest2.RandomWithPrefix("test-acc-scaleway-project")
	iamApplicationName := acctest2.RandomWithPrefix("test-acc-scaleway-iam-app")
	iamPolicyName := acctest2.RandomWithPrefix("test-acc-scaleway-iam-policy")
	...
	iamAPI := iamSDK.NewAPI(tt.Meta.ScwClient())
	iamApplication, err := iamAPI.CreateApplication(&iamSDK.CreateApplicationRequest{
	...
	terminateFunctions = append(terminateFunctions, func() error {
		return iamAPI.DeleteApplication(&iamSDK.DeleteApplicationRequest{
			ApplicationID: iamApplication.ID,
		})
	})

	iamPolicy, err := iamAPI.CreatePolicy(&iamSDK.CreatePolicyRequest{
		Name:          iamPolicyName,
		ApplicationID: types.ExpandStringPtr(iamApplication.ID),
		Rules: []*iamSDK.RuleSpecs{
			{
	...
	terminateFunctions = append(terminateFunctions, func() error {
		return iamAPI.DeletePolicy(&iamSDK.DeletePolicyRequest{
			PolicyID: iamPolicy.ID,
		})
	})

	iamAPIKey, err := iamAPI.CreateAPIKey(&iamSDK.CreateAPIKeyRequest{
		ApplicationID:    types.ExpandStringPtr(iamApplication.ID),
	...
	terminateFunctions = append(terminateFunctions, func() error {
		return iamAPI.DeleteAPIKey(&iamSDK.DeleteAPIKeyRequest{
